...
{
    public function consistate()
    {
        $properties = Property::all();
        foreach ($properties as $property) {

            $size = (float) $property->size;
            $GLA = (float) $property->GLA;
            $price = (float) $property->price;
            
            $property->size_mt = round($size * 0.092903, 2);
            ...
            if ($property->listing_type == 1) {
                $property->avg_ft = round($price / $size, 2);
                $property->avg_mt = round($price / ($size * 0.092903), 2);
            }

            $property->agent = rand(1, 10);
            $property->save();
        }
        return 'DONE';
    }
    public function changeCityThing()
    {
        $change = Neighborhood::query()->update(['city_id' => 1]);
        if ($change) {
            return 'DONE';
        }
    }
