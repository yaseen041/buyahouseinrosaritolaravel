...
<div class="row wrapper border-bottom white-bg page-heading">
    ...
    <div class="col-lg-4 col-sm-4 col-xs-4 text-right">
        <a class="btn btn-primary text-white t_m_25" data-toggle="modal" data-target="#add_modalbox">
            <i class="fa fa-plus" aria-hidden="true"></i> Add New Htaccess
        </a>
    </div>
</div>
        ...
        <div class="col-lg-12">
            <div class="ibox">
                <div class="ibox-content">
                    <form id="search_form" action="{{url('admin/htaccess')}}" method="GET" enctype="multipart/form-data">
                        <div class="form-group row justify-content-end">
                            <div class="col-sm-6">
                                <div class="input-group">
                                    <input type="text" class="form-control" name="search_query" placeholder="Search by Content" value="{{ old('search_query', $searchParams['search_query'] ?? '') }}">
                                    <span class="input-group-append">
                                        <button type="submit" class="btn btn-primary">Search</button>
                                        <a class="btn btn-danger ml-4" href="{{ url('admin/htaccess/') }}">
                                            Clear Search
                                        </a>
                                    </span>
                                </div>
                    ...
                    <div class="row">
                        <div class="col-md-9">
                            <p>Showing {{ $htaccesses->firstItem() }} to {{ $htaccesses->lastItem() }} of {{ $htaccesses->total() }} entries</p>
                        </div>
                        <div class="col-md-3 text-right">
                            {{ $htaccesses->links('pagination::bootstrap-4') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content animated flipInY">
            <div class="modal-header">
                ...
                <h5 class="modal-title">Add New Htaccess</h5>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="save_testimonial_button"> Submit </button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).on("click", "#save_testimonial_button", function() {
        var btn = $(this).ladda();
        btn.ladda('start');
        var formData = new FormData($("#add_testimonial_form")[0]);
        $.ajax({
            url: "{{ url('admin/htaccess/store') }}",
            type: 'POST',
            data: formData,
            dataType: 'json',
            cache: false,
